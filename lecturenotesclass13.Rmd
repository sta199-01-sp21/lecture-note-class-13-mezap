...
```{r data_manhattan}
manhattan <- read_csv("~/lecture-note-class-13-mezap/manhattan.csv")
```
...
This means that we are 95% confident that the mean price of 1-bedroom apartments
in Manhattan is between 2299 and 2982 dollars. 
...
Interpretation: We are 95% confident that the median price of a 1-bedroom apt
in Manhattan is between 2162 and 2875.
...
**Interpretation**: We are 95% confident that the proportion of mice that died
while not on treatment is in the interval of 0.69 to 0.793. 
...
```{r practice_2_ci}
ci_mice_lived <- get_ci(boot_prop2, level = .90)
ci_mice_lived
```
...
We are 90% confident that the proportion of mice that lived while on the 
treatment is between 0.349 and 0.448.
